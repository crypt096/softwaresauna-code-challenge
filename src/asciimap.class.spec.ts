...
describe('ASCIIMap class', () => {
  describe('Constructor', () => {
    ...
    test('should split map data on new line', () => {
      ...
      expect(asciiMap.MapMatrix[1].length).toBe(3);
    });

    test('should handle trailing newline character', () => {
      const mapData = `---\n---\n`;
      const asciiMap = new ASCIIMap(mapData);
      expect(asciiMap.MapMatrix.length).toBe(3);
      expect(asciiMap.MapMatrix[2].length).toBe(0);
    });
  ...
  describe('function isPositionWithinMap', () => {
    ...
    test('should return false if position outside of map', () => {
      const mapData = `---\n---`;
      const asciiMap = new ASCIIMap(mapData);
      const testPosition1: Position = { x: 5, y: 5 };
      const isPosition1OnMap = asciiMap.isPositionWithinMap(asciiMap, testPosition1);
      expect(isPosition1OnMap).toBe(false);

      const testPosition2: Position = { x: -1, y: -1 };
      const isPosition2OnMap = asciiMap.isPositionWithinMap(asciiMap, testPosition2);
  ...
  describe('function getCharacterAtPosition', () => {
    ...
    test('should return character at map position', () => {
      const mapData = `--@--\n-x----`;
      const asciiMap = new ASCIIMap(mapData);
      const testPosition1: Position = { x: 2, y: 0 };
      const testPosition2: Position = { x: 1, y: 1 };
    ...
    test('should return null if position is outside of map bounds', () => {
      const mapData = `--@--\n-x----`;
      const asciiMap = new ASCIIMap(mapData);
      const testPosition1: Position = { x: 12, y: 10 };
      const testPosition2: Position = { x: -11, y: -11 };
